...
| `server` | `test/server` | `repo.ts` and the schema against a real Postgres, including booting on top of a pre-migration database. |
...
Claude Code / Codex account). Permissions are still covered end to end, because
a permission is a row — `answerPermission` resolves it with no adapter attached.
