...
- **Inside a dev environment, `docker` is the host's daemon through Domo's
  proxy**, scoped so the environment sees only what it made. It is not a
  sandbox. While Domo restarts, a `docker` call there hangs instead of failing
  (Docker Desktop), and completes when Domo is back.
...
- `docs/dev-environments.md`: read before you change the environment
  lifecycle or the Docker proxy (`server/lib/dood/`), or when `docker` in an
  environment acts in an unexpected way.
