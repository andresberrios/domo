...
- **Steer** while a turn is in flight — sending mid-turn injects the message into the live `claude` (picked up at the next step boundary, turn continues). See [Steering a running turn](#steering-a-running-turn-mid-turn-input) (Decided #18).
...
1. **Pending agent edit.** When the agent's `Edit`/`Write` tool fires through the IDE bridge's `openDiff` call, the entity parks the call and emits a `pending_diff` event + a durable `pendingDiffs` row `{ callId, path, before, after, status }`. The chat renders an inline approval card (reusing `DomoDiffView`) and links to the workspace surface in `diff` mode for the full view. The user clicks accept / reject; the entity resolves the parked call with `FILE_SAVED` or `DIFF_REJECTED`.

> **✅ Step-11 trigger: `--permission-prompt-tool stdio` (resolved & verified e2e).** The original assumption — that headless `claude -p` routes `Edit`/`Write` through the IDE-bridge `openDiff` — is **false** (`claude` 2.1.142): under *either* `acceptEdits` or `default` the CLI permission-denies the tool, `openDiff` is never called. The correct mechanism is the one the **official VS Code extension** uses (confirmed from the user's running-process command line and the public Claude Code source snapshot, `src/cli/structuredIO.ts`): run with **`--permission-prompt-tool stdio --permission-mode default`**. Each tool the CLI wants to run emits a `control_request` `{ subtype:'can_use_tool', tool_name, input, tool_use_id }` NDJSON line on **stdout**; the host answers with a `control_response` `{ subtype:'success', request_id, response:{ behavior:'allow', updatedInput } | { behavior:'deny', message } }` on **stdin**. stdin stays open for the whole turn (closed on the `result` envelope). On *allow* the **CLI itself performs the edit** — Domo never writes the file in the live path. This **supersedes the IDE-bridge `openDiff`** trigger (design Decided #15): `server/lib/electric/bridge.ts` is now dormant (not booted per turn; kept for possible future editor-context tools). `claude.ts` carries the protocol; `entity.ts` `executeClaudeTurn.onPermissionRequest` is the policy: **non-edit tools auto-allow** (Bash/Read stay frictionless, as `acceptEdits` used to), **edit-family tools** (`Edit`/`Write`/`MultiEdit`/`NotebookEdit`) become a durable `pendingDiffs` row + approval card.

**Resolution control flow (built & verified live e2e — accept, reject, and server-restart resume).** The pull-wake runner is single-flight per entity: a `prompt` turn blocked awaiting a permission decision is still inside its handler, so a `diff_decision` *wake* would never be claimed (deadlock). Since the runtime is **in-process**, `sessions.diffDecision` resolves the parked promise directly (`server/lib/electric/sessionControl`); the still-running turn answers the CLI `allow`/`deny` and records the outcome durably (`pendingDiffs` status + `diff_decision` event). **Durability/resumability:** the `pendingDiffs` row lives in the durable stream (agents-server Postgres), so the approval card **replays to any client and after a server restart** (verified: kill the server mid-park, restart, reload → the card is back). A server restart kills the in-process park *and* the `claude` child, so the interrupted turn is dead. `reconcileStalePendingDiffs` (run at the top of every handler invocation — safe because the single-flight runner means a *live* park only ever exists inside the currently-executing invocation) auto-rejects any orphaned `pending` row so the stale card clears deterministically; the interrupted `prompt` (its inbox key was never advanced) re-runs on the next wake, re-proposing a **fresh, actionable** diff card. The user reviews & accepts that, the CLI applies the edit, and the conversation continues via `--resume` (nativeSessionId is durable in `sessionMeta`). The honest bound: the *exact* dead turn can't resume mid-tool (claude `-p` is one-shot), but the pending review is never lost — it's re-surfaced and actionable, which is "review later & continue from there".
...
> **Superseded.** The two subsections below described the IDE-bridge
> `openDiff`-based approval and a `--permission-prompt-tool <mcp>`
> fallback. Step 11 proved headless `claude -p` does not use `openDiff`;
> the shipped mechanism is **`--permission-prompt-tool stdio`** (the
> built-in stdio value, not a custom MCP) — exactly what the official VS
> Code extension does. See the ✅ box under
> [Diff visualization](#diff-visualization-two-cases) for the resolved
> design. The text below is retained only for historical context.

**For tools without a bridge entry** (Bash, MultiEdit, etc.): ~~we launch with `--permission-mode acceptEdits`~~ → we launch with `--permission-mode default --permission-prompt-tool stdio`; non-edit tools are auto-allowed by `onPermissionRequest`, edit-family tools become approval cards.

#### Fallback (historical): `--permission-prompt-tool` via custom MCP

`--permission-prompt-tool` accepts the literal `stdio` (control-message protocol over the stream-json channel — what we ship) **or** an MCP tool name (a custom MCP server exposing `approval_prompt(tool_name, input, tool_use_id)` → `{ behavior:"allow", updatedInput } | { behavior:"deny", message }`, the andrepimenta [claude-code-chat](https://github.com/andrepimenta/claude-code-chat) `claude-code-chat-permissions-mcp/` pattern). We use the `stdio` form: no extra server, identical semantics, and it's the official-extension path.
...
### Steering a running turn (mid-turn input)

**Goal.** Send a message to a working agent that it picks up *while it continues*, not only after the turn ends and not as an interrupt — "actually, also do X" / "stop chasing that, focus on Y".

**Spike findings** (`smoke/steering-spike.mjs`, real `claude` 2.x, our exact headless flags + `--replay-user-messages`, injected a user message mid-`Bash`-tool):

- `--replay-user-messages` (per `main.tsx`: *"Re-emit user messages from stdin back on stdout for acknowledgment; only with --input-format=stream-json and --output-format=stream-json"*) makes the CLI echo every user message on **stdout** as `{type:'user', message, uuid, isReplay:true}`.
- A user message written to the open stdin mid-turn is **queued** (`messageQueueManager`, priorities `now>next>later`) and **consumed at the next step/tool boundary** — the agent then redirects and the turn continues (spike: injected at +5.98s during an 8 s `sleep`, picked up at +12.75s the instant the tool returned, model abandoned the remaining steps and complied). True steering, not interrupt, not end-of-turn-only.
- The replay is a **consumption ack**, not a receipt ack: it arrives *when the agent drains the queue at the boundary* (spike: 6.78 s after send), interleaved in stdout order. So latency from send→ack = "time until the next boundary" (bounded by the in-flight tool's duration).

**Decision (Decided #18).** Steering is an **in-process side-channel**, exactly the shape diff-decisions already use (`sessionControl.ts`) — the single-flight pull-wake runner can't deliver a wake mid-turn:

- Spawn gains `--replay-user-messages`. `runClaudeTurn` exposes the live child via `onReady(steer)`; `executeClaudeTurn` registers that steer fn in `sessionControl` for the session's lifetime (cleared in `endTurn`).
- `sessions.prompt`: generate a `uuid`; try `steerSession(id,text,uuid)`. **If a turn is live** → the registered callback (a) appends a durable `steer_sent` event `{text,uuid}` *from inside the live handler* (same way diff events are written) and (b) writes `{type:'user',message,uuid}` to the child's stdin; return `{steered:true,uuid}`. **If not live** → unchanged (durable `prompt` inbox → new turn).
- The transcript is durable via the **event stream only** — no new inbox type, so no single-flight / double-processing / watermark problem. The adapter renders `steer_sent` events as user bubbles; the CLI's `isReplay` echo (mirrored to `events` like every stdout envelope) is matched **by `uuid`** to flip the bubble `queued → delivered`. `isReplay` user events are *never* rendered as bubbles (that also sidesteps the initial-prompt-echo: the first prompt's replay is ignored; its bubble comes from the inbox as before).
- **No `@`/slash expansion for steered text in v1** (steering is normally plain English). This keeps the raw-text invariant trivially (nothing is expanded, so `steer_sent` == what the user typed == what the CLI echoes) and avoids reintroducing the inbox path. Documented limitation.
- **Restart semantics.** Best-effort, *not* a durable queue (unlike diff-approval, which we deliberately kept durable). A `steer_sent` with no matching replay (server died between append and ack) shows as a stale "queued" bubble; the turn is dead and the interrupted prompt re-runs via the existing reconcile path. Acceptable for a steering nicety; a `reconcileStaleSteer` could mark it later if it bites.

UX is already mostly present: `DomoChatInput`'s Enter-to-send works while `status==='streaming'` and the placeholder already says so — only the copy ("steer") and the queued/delivered badge are new.
...
(Decided #19.) Domo is **a host-installed app + a small compose'd infra stack**, not a docker-compose-only product — Decided #11 makes the app a host-side orchestrator, so containerising it would reintroduce the Option-B credential/OAuth risk.

**Two layers:**

| | Ships as | Why |
|---|---|---|
| Infra (Postgres + `@electric-ax/agents-server`) | pinned `docker-compose.yml`, **published version-tagged images** | no host-env needs; clean to containerise. (The current repo bind-mount of `.` into the agents-server container is a *dev* shortcut — releases need a self-contained image or a host `node …/agents-server/dist/entrypoint.js`.) |
| Domo app | host install: `curl … \| sh` + a `domo` CLI, running the Nitro `node-server` build | must inherit the user's env / `~/.claude` / host `git` / project files / localhost `coastd` |

The `domo` CLI manages the infra for the user (`domo up` → `docker compose up -d` + run the host server via a systemd/launchd unit), so it's still effectively one command — just not *raw* `docker compose up`.

**Release process.** Maintainer steps in [`docs/site/releasing.md`](./site/releasing.md). In short: bump `package.json`, tag `v*`, push → `.github/workflows/release.yml` builds the per-platform tarball (incl. bundled Node), `install.sh`, and `SHA256SUMS` on a matrix and attaches them to the GitHub release. The installer prereq-checks **Docker, Coast, `claude` (logged in), git** (Node is bundled), installs to `<domoHome>/app/<ver>` (+ `current` symlink), drops the `domo` CLI; `domo update` re-runs the installer and restarts if running.

**Customising claude's environment.** Because `claude` runs in the Domo process env, this is install-method-agnostic. `<domoHome>/config.json` (operator-facing host config; `server/lib/config.ts`, distinct from the SQLite UX-prefs store) supports:

```json
{ "claude": { "env": { "FOO": "bar" }, "extraPath": ["/opt/lang/bin"] } }
```
...
**Status: built; current release v0.1.4** — the only published release. v0.1.0–v0.1.3 tags/releases were deleted: the v0.1.2/v0.1.3 release runs hung indefinitely on GitHub's `macos-13` Intel runner (retired 2025-12-04) since `release` is `needs: build`, so nothing ever published. v0.1.4 is the first release with a fully green CI matrix (fix: `darwin-x64` → `macos-15-intel`; see the CI-runner refinement below). `scripts/install.sh` (curl|sh, OS/arch-detecting, checksum-verified, `DOMO_LOCAL_TARBALL` for offline), `bin/domo` (the CLI: `up`/`down`/`status`/`logs`/`update`/`version`), `release/docker-compose.yml` + `release/Dockerfile.agents-server`, `scripts/build-release.sh` (→ `dist/` tarball + installer + `SHA256SUMS` + `manifest.json`), and `.github/workflows/release.yml` (tag `v*` → **matrix** build + attach). Refinements vs. the original sketch:

- **agents-server is not a pre-published registry image** — `release/docker-compose.yml` builds it from a pinned Dockerfile on the first `domo up` (one-time, ~1–2 min; reproducible from the `package.json` pins, no registry/CI-auth needed). Pre-publishing to GHCR stays a later optimization.
- **Multi-platform.** Tarballs are per `os-arch` (native `better-sqlite3` is bundled into `.output`): `linux-x64`, `linux-arm64`, `darwin-arm64`, `darwin-x64`, each built on a matching CI runner. **WSL is just Linux** (the linux tarball; the installer prints a Docker-Desktop-WSL note). `install.sh` detects and downloads the match. (Only `linux-x64` is locally verifiable in the dev box; the rest are CI-produced.)
- **Bundled Node.** `build-release.sh` resolves the latest Node 22 LTS, checksum-verifies it (`SHASUMS256.txt`), and ships just the `node` binary at `runtime/bin/node`; `bin/domo` runs the app with it (system Node not required — verified e2e that `/proc/<pid>/exe` is the bundled binary). Tarball ≈ 50 MB (was ~7 MB). Remaining *host* requirements are **Docker Compose, Coast, git, and the logged-in `claude` CLI** — `git`/`claude` can't be bundled away because Domo orchestrates them host-side (Decided #11), so "only Docker + Coast" isn't fully reachable; Node was the bundleable one.
- **Infra runs as the host UID.** Discovered by running `domo up` for real: bind-mounted Postgres/streams data was root/container-owned → `rm -rf`/`tar` of `$DOMO_HOME` failed. Fix: compose `user: ${DOMO_UID}:${DOMO_GID}` (set by the CLI) on both services + `chmod 0777 /app` in the agents-server image (it `mkdir`s `/app/logs` at startup, can't as non-root otherwise). Verified: data is host-owned, backup/wipe work.
- **Postgres is not published to the host** — agents-server reaches it over the compose network and the Domo app never touches Postgres (Decided #14), so there's no host port to clash with a user's own Postgres. (Refinement of the user's "custom port" idea — *no* port is cleaner than a custom one.)
- **All data lives under `$DOMO_HOME`** (default `~/.domo`): `state.db`, `app/<ver>` + `current`, `run/`, and Postgres + durable streams as **bind mounts** under `$DOMO_HOME/data` (`DOMO_DATA_DIR`, set by the CLI; compose uses `${DOMO_DATA_DIR:?}` so a bare `docker compose` fails loudly). Not Docker named volumes — the whole install is one dir to `tar` (back up) or `rm -rf` (wipe).
- **CI runner reality (post-v0.1.3).** The `darwin-x64` matrix leg builds on **`macos-15-intel`** — `macos-13` (the prior free Intel runner) was retired 2025-12-04 and a job on a dead runner label hangs in `queued` forever rather than failing, which is what silently wedged v0.1.2/v0.1.3. `macos-15-intel` is GitHub's **last** x86_64 macOS image, available **until 2027-08**; past that, `darwin-x64` must be dropped or cross-built from an arm64 mac (Apple/GitHub are ending x86_64 macOS support). The release actions were also bumped off Node-20 (`checkout@v6`, `pnpm/action-setup@v6`, `setup-node@v6`, `upload-artifact@v7`, `download-artifact@v8`; Node 20 enforced 2026-06-02). Maintainer runbook: `docs/site/releasing.md`.
...
18. **Mid-turn steering is an in-process side-channel + `--replay-user-messages`; best-effort, event-stream-durable, no new inbox type.** Surfaced post-Phase-4 from a user request, validated by a real-CLI spike (`smoke/steering-spike.mjs`): a stdin user message mid-turn is *queued* and consumed at the next step/tool boundary (the agent redirects, the turn continues), and `--replay-user-messages` echoes it on stdout as a consumption ack. `sessions.prompt` routes a send to `steerSession()` when a turn is live (registered in `sessionControl`, the same in-process channel diff-decisions use because the single-flight pull-wake runner can't deliver a wake mid-turn); the live handler appends a durable `steer_sent` event and writes the message to the child's stdin. The transcript stays durable purely through the event stream (no inbox `steer` type → no double-processing / watermark issue): the adapter renders `steer_sent` as a user bubble and matches the CLI's `isReplay` echo by `uuid` to flip `queued→delivered`. No `@`/slash expansion for steered text in v1 (keeps the raw-text invariant trivially). Best-effort across restart (contrast diff-approval, kept durable). Full rationale + spike timeline: [Steering a running turn](#steering-a-running-turn-mid-turn-input).

19. **Distribution: Domo is a host-installed app + compose'd infra — NOT a docker-compose-only stack.** Decided #11 makes Domo a host-side orchestrator: it `spawn('claude')` in its own process env, shells host `git`, reads/writes host project files at `<root>/.worktrees/*`, talks to host `coastd` on localhost (which itself runs DinD), and relies on the user's `~/.claude` for subscription auth + MCP. Containerising the *app* would force mounting `~/.claude` (the exact credential-exposure + OAuth-replay risk that sank Option B — now on the Domo container), the docker socket, every project dir, and host git — the opposite of "minimal config" and a security regression. So the split is: **infra → a pinned compose stack** (Postgres + an agents-server image **built at first `domo up`** from a pinned Dockerfile — no registry); **app → a host install** (`curl … | sh` installer + a `domo` CLI) with a **bundled Node**, inheriting the user's env / `~/.claude` / git / projects / `coastd`. This *is* the "VPS five-minute path." Customising claude's env is install-method-agnostic — `<domoHome>/config.json` `claude.env`/`claude.extraPath`, merged **before** the security scrub re-applies (Decided #9 stays final; cannot reintroduce `ANTHROPIC_API_KEY`). **The "Status: built" bullets in [Distribution & release](#distribution--release) are the authoritative current state; this paragraph is the rationale.**

20. **Multi-user auth = `nuxt-auth-utils` email+password, no email sent, first-run admin + admin-approval; the boundary is one Nitro middleware.** Surfaced post-Phase-4 from a user request (secure the install once exposed beyond localhost *and* enable a team to share it). OAuth deferred (a future managed/SaaS Domo can centralize it; FSL-1.1 already blocks third parties running Domo as a managed service). Storage mirrors the operea project's nuxt-auth-utils pattern but on Domo's raw better-sqlite3 (a `users` table via the existing `CREATE TABLE IF NOT EXISTS` migrate block — *not* operea's ZenStack/Postgres). **No email is sent**: the first account created is the admin (`role=admin, status=active`, logged straight in); later signups are `role=member, status=pending` and parked on `/pending` until the admin approves at `/admin/users`. The **sealed cookie holds identity only** (`{id,email,name}`); `role`/`status` are re-read from the DB in every server guard, so approve/reject lands on the next request with **no re-login** (a sealed cookie can't be mutated server-side) — rejected the alternative of caching role/status in the cookie (stale-permission window). **Enforcement is `server/middleware/auth.ts`**, a single Nitro middleware over `/procedures/**` + Domo's own SSE/WS `/api/*` + the `/_agents/**` durable-stream proxy, allow-listing only bootstrap/setup/register/login/me — chosen over sprinkling `requireUserSession` into ~40 procedures (defence-in-depth `requireAdmin` still lives in the admin procedures). It must **not** gate the broad `/api/` prefix (framework `/api/_auth/`, `/api/_nuxt_icon/` live there). The session secret is **auto-managed** under `$DOMO_HOME/session-secret` by a Nitro plugin (operator sets nothing; `NUXT_SESSION_PASSWORD` overrides) — consistent with Decided #19's "Domo self-manages everything under `$DOMO_HOME`"; in dev nuxt-auth-utils' own `.env` auto-gen takes precedence (the plugin owns only the production path). SPA: the dashboard chrome was extracted to `DomoAppShell` and mounts only for a signed-in active user so its data procedures never fire pre-auth. Verified live e2e (isolated `DOMO_HOME`, 0 console errors, CSS rendered). Access model v1: any active user sees all sessions (per-session ACLs deferred). Built & shipped — see CLAUDE.md "Multi-user auth" + `docs/tasks/phase-5-collab.md`.

21. **Group-chat collaboration: a chat message does NOT trigger the agent — only `@agent` or a "Send to agent" button does (DESIGNED, NOT BUILT).** The collaboration half of the same user request. Today every `prompt` inbox message runs a `claude` turn; the new model decouples "a user posted" from "the agent runs". Chosen architecture: a new durable **`chat`** inbox/event type carrying `{text, author:{userId,userName}}`; the entity records every chat message to the durable stream (group-visible, authored) but only calls `executeClaudeTurn` when the message contains an `@agent` mention **or** carries an explicit `trigger:true` (the button). On trigger it folds the un-consumed chat backlog (since the last turn boundary) into the synthesized prompt — the per-message raw `chat` events stay in the transcript (raw-text invariant preserved; trigger detection lives in the entity, like `@`/slash expansion). Mid-turn `@agent` reuses the existing steering side-channel (Decided #18). Identity is injected by the (authenticated) procedure and trusted by the in-process entity. **The fork — durable stream vs separate SQLite table + a new realtime channel — was resolved toward the durable stream** (single source of truth, restart-safe, the adapter already merges multiple event types; cost = a bump to the "locked" `electric/schemas.ts`, an accepted dev-throwaway tradeoff). Identity on `chat` events extends those schemas; `app/utils/sessionMessages.ts` + `DomoChat` render authored human bubbles distinctly from the agent. Tracked in `docs/tasks/phase-5-collab.md`.

22. **Per-session edit-approval policy (`manual` / `auto` / `passthrough`); default operator-set.** Surfaced post-Phase-4 from a user request (they run the `claude` CLI with `permissions.defaultMode:"auto"` and want that workflow in Domo). `manual` = the original Phase-3 behavior (every edit parks as a diff-approval card). `auto` = `claude` spawned `--permission-mode acceptEdits` (no park). `passthrough` = Domo does **not** pass `--permission-mode`, so the user's own `~/.claude/settings.json` (`defaultMode` + its model classifier) decides (no park). Resolved at the start of each turn as `session.approvalMode ?? config.claude.approvalMode ?? 'manual'` — a plain Domo-side read (`sessions` row + `<domoHome>/config.json`), deliberately **not** a durable inbox round-trip, so a change applies next turn with no wake and no restart-resume exposure. Only `manual` ever parks → `auto`/`passthrough` are inherently restart-safe (no `pendingDiffs` row to orphan). Storage: a nullable `approval_mode` column on the `sessions` table (idempotent `ensureColumn` migration; null = inherit), `ApprovalMode` Zod enum in `server/lib/schemas.ts`, `claude.approvalMode` in `server/lib/config.ts`, `sessions.setApprovalMode` procedure, and a `USelect` in `DomoChat`. Rejected: storing the mode on the durable `sessionMeta` (would re-introduce the wake dependency for a pure UX setting) and a global-only switch (per-session override is the common need).

23. **Restart-resume = app-only restart (the Horton-aligned common path) + a vendored agents-server boot-relink patch (the agents-server-restart path).** Investigation (prompted by a user-reported orphaned parked diff) established the real mechanism, from upstream source: `@electric-ax/agents-server@0.4.2` keeps pull-wake **subscriptions in memory only** (`SubscriptionManager` is a `Map`) and does **not** rebuild them on its own boot (`rebuildWakeRegistry` reloads only wake *conditions*; streams/offsets persist, subscriptions don't). Official "clean restart" therefore only covers the **runner** restarting while agents-server stays up (the runner heartbeats its `wake_stream_offset`; re-registration preserves it; it resumes and replays backlog wakes). Two fixes: **(a)** `bin/domo restart` is now an **app-only** swap (`cmd_restart` keeps postgres+agents-server running; full teardown is only explicit `domo down`) — so `domo update` / `update:local` no longer gratuitously recreates agents-server, and offset-replay just works (this was the bulk of the self-inflicted breakage; an earlier `restart`=`down`+`up` recreated agents-server every update). **(b)** For genuine agents-server restart (crash / host reboot / version upgrade / image change), a vendored patch (`release/agents-server-0.4.2-boot-relink.patch`) adds the missing boot step: after `rebuildWakeRegistry`, list all persisted non-stopped policy-bearing entities and call agents-server's own `linkEntityDispatchSubscription` for each (mirrors exactly what `spawn` does). Applied idempotently by `scripts/apply-patches.sh` (dev `postinstall`; pnpm 11.0.9 would not reconcile `patchedDependencies` here and the release image installs with npm anyway) and by `release/Dockerfile.agents-server` (npm-install path). Verified live: spawn → `--force-recreate` agents-server → boot log `re-linked dispatch subscriptions for 1 persisted entity` → subscription re-linked → a fresh send emits a wake (`smoke/agents-server-restart-resume.mjs`, isolated stack). Rejected: a host-side claim/boot sweep (claim 404s when the subscription is gone — it cannot work after an agents-server restart) and hand-rolling the durable-streams subscription-proxy path math (fragile internal coupling). Upstream PR target: `electric-sql/electric` (`packages/agents-server`); the vendored patch is removed once merged.
