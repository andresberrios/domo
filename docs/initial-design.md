...
- `../nuxt-chat-template/` — [`nuxt-ui-templates/chat`](https://github.com/nuxt-ui-templates/chat). Streaming AI chat built on Nuxt UI's `UChat*` components and the Vercel AI SDK. We borrow its layout shell, `UChat*` primitives, `MessageContent.vue` part-rendering pattern, and `Comark` markdown setup. We replace its transport and message-shape.
...
- `../electric-source/` — Electric Agents source. The removed `coding-session.ts` entity at commit `65f0cf0` is the reference shape for our `claude-code-cli` entity.
...
The chat template uses the Vercel AI SDK's `Chat` class + `DefaultChatTransport` pointed at `/api/chats/:id`, which internally calls `streamText({ model, ... })`. We replace that with **an Electric Agents durable-stream subscription** projecting an `events` collection into the `UIMessage.parts` shape `UChatMessages` already understands. Sending a prompt is a runtime `.send()` on the entity's inbox. See [Session runtime](#session-runtime-electric-agents-with-the-claude-code-cli-entity).
...
Dropped (vs the earlier draft):

- The Docker engine API integration / Caddy ingress / per-env compose orchestration code — Coast does all of this.
...
1. **Where `claude` runs — host-side vs in-coast.** Recommendation: host-side for v1, revisit after first users; see [Where claude runs](#where-claude-runs-pending-decision).
...
1. **Wire-level shape for the `claude-code-cli` entity.** Specifically: the exact JSON schema we use for `events` rows (so the UI's `UIMessage.parts` projector is stable), the schema for `pendingDiffs`, and how `diff_decision` inbox messages reference the parked WS call id.
