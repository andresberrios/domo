...
**Release process.** Git tag → CI builds: (a) the app tarball (`pnpm build` → `.output/`), (b) the pinned `docker-compose.yml`, (c) a **manifest** mapping the Domo version → compatible agents-server / Postgres image tags + the pinned `@durable-streams` build, (d) a checksummed/signed `install.sh`. The installer prereq-checks **Docker, Coast (pinned range — Pending #4), `claude` (logged in), git, Node (or bundled)** with actionable remediation, installs to `<domoHome>/app`, drops the `domo` CLI, brings up infra, starts the host server. `domo update`: fetch + verify the release, atomic app-dir swap, `docker compose pull && up -d` per the manifest, restart. Versions are pinned and move together (app ↔ infra image tags ↔ durable-streams build).
...
19. **Distribution: Domo is a host-installed app + compose'd infra — NOT a docker-compose-only stack.** Decided #11 makes Domo a host-side orchestrator: it `spawn('claude')` in its own process env, shells host `git`, reads/writes host project files at `<root>/.worktrees/*`, talks to host `coastd` on localhost (which itself runs DinD), and relies on the user's `~/.claude` for subscription auth + MCP. Containerising the *app* would force mounting `~/.claude` (the exact credential-exposure + OAuth-replay risk that sank Option B — now on the Domo container), the docker socket, every project dir, and host git — the opposite of "minimal config" and a security regression. So the split is: **infra → a pinned `docker-compose.yml`** (Postgres + agents-server; today's repo bind-mount of `.` into the agents-server container is a *dev* shortcut and must become a published, version-tagged image for releases); **app → a host install** (`curl … | sh` installer + a `domo` CLI: `up`/`update`/`logs`/`status`) running the Nitro `node-server` build, which inherits the user's env and sees `~/.claude`/git/projects/`coastd`. This *is* the long-assumed "VPS five-minute path." Release = git tag → CI artifacts (app tarball + pinned compose + a version→infra-image-tag manifest + a checksummed installer); `domo update` = verify + atomic app-dir swap + `docker compose pull && up -d` per the manifest; everything version-pinned and moves together. Customising claude's env is install-method-agnostic and small — an operator `<domoHome>/config.json` `claude.env`/`claude.extraPath` merged into the spawn **before** the security scrub re-applies (Decided #9 stays the final word; cannot reintroduce a stripped `ANTHROPIC_API_KEY`). See [Distribution & release](#distribution--release).
