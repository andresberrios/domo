...
22. **The `claude` spawn mirrors the official VS Code extension EXACTLY (no `-p`; `CLAUDE_CODE_ENTRYPOINT=claude-vscode`) — billing self-preservation, surfaced 2026-05-19.** Anthropic support article 15036540: **after 2026-06-15**, `claude -p` / Agent-SDK usage leaves the full Claude subscription for a small *capped monthly credit* (Pro $20 … Max20x $200); **interactive Claude Code (no `-p`) stays on the full subscription**. Spike (`smoke/no-print-lifecycle-spike.mjs`) proved the billing classifier reads the outbound `x-anthropic-billing-header: cc_entrypoint=`, which `claude`'s `main.tsx initializeEntrypoint()` takes from `CLAUDE_CODE_ENTRYPOINT` (**preserved if set**, else forced `sdk-cli` for piped/non-TTY) — and that **`-p` is a red herring** (changes neither billing nor lifecycle; the no-`-p` process still exits cleanly on stdin-close, so the spawn-per-turn model holds). This **supersedes the Phase-3 step-11 framing** ("headless `-p` + `--permission-prompt-tool stdio`"): step-11's *mechanism* (stdio permission, not IDE-bridge `openDiff`; `bridge.ts` dormant) is unchanged and still matches the extension (its argv carries `--permission-prompt-tool stdio` too), but `-p` is dropped. Phase 1 (shipped 2026-05-19): `server/lib/electric/claude.ts` sets `CLAUDE_CODE_ENTRYPOINT=claude-vscode` (scrubbed first for nested-claude hygiene, then pinned) and passes the extension's **exact new-session argv minus `-p`** (incl. `--max-thinking-tokens 31999 --setting-sources=user,project,local --include-partial-messages --debug --debug-to-stderr --enable-auth-status --no-chrome`; **no `--add-dir`**; conditional `--resume` already matched the extension's resume-vs-new behavior). Anti-spoofing rationale (the user's call): genuinely *use* the CLI the allowed/interactive way, full argv fidelity, not a header that contradicts behavioral telemetry. Phase 2 (the **long-lived per-session `claude` process** — one process for the session's lifetime, multi-turn over one persistent stdin, idle-reap + restart-resume) is the remaining behavioral-fidelity work; tracked in `docs/tasks/phase-5-collab.md`/CLAUDE.md + the `project-agent-sdk-billing` memory. Sandbox-litter / pnpm-`.bin` is a *separate* issue (operator `~/.claude` `defaultMode:auto` → Claude Code's bwrap sandbox), stop-gapped by a `.gitignore` block, real fix TBD.
