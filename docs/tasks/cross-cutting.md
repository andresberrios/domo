...
- [x] **9. No-remote projects.** **Supported today** — `projects.add` never touches git remotes; any local path works, and a non-repo path is offered `git init` (no `origin` required). The only **post-v1** piece is the *clone-a-remote-URL-from-Domo's-UI* convenience flow; until then the user provisions the repo on the VPS however they like and points Domo at the path (`initial-design.md` line ~169).
...
- [x] `docs/site/getting-started.md` (VPS five-minute path) — **drafted** against the distribution model (`initial-design.md` Decided #19): host-installed app + compose'd infra, prereqs, the `<domoHome>/config.json` `claude.env` knob. Installer/`domo` CLI/CI are still specced-not-built.
