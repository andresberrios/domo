...
| --- | --- | --- |
| `unit` | `test/unit`, `test/docker` | plain node, no services, no Nuxt. Pure logic (`buildTranscript()`, formatters, settings reconciliation, `.domo.json` parsing and validation, the generated build config, the image-metadata allow-list, the `docker run` argv and the runtime volume's name, the voice tools with everything below them mocked, the usage normalisers and the poller's scheduling with injected clients) plus Docker at the process boundary — the exact argv handed to `docker`, which needs no daemon. |
