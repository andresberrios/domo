...
/**
 * What an environment made through its Docker proxy. The agent named it, so no
 * row can name it back: what attributes it is the `domo.env` label the proxy
 * stamped on it, and the `domo-<id>/` spelling of a tag it produced.
 */
describe('what an environment made on the host daemon', () => {
  const MADE = {
    containers: [
      { name: 'env_1-web', owner: 'env_1' },
      { name: 'env_live-web', owner: 'env_live' },
      // The one helper every environment's published ports go through.
      'domo-dev-port-helper'
    ],
    ...
    images: [
      ...
      { name: 'domo-env_live/docker.io/library/app:dev', owner: 'env_live' },
      'domo-dev-port-helper:0123456789ab'
    ]
  }
  const LIVE = environment({ id: 'env_live' })

  it('is claimed by its label once the environment is retired, container before network before volume', () => {
    expect(planned({ environments: [RETIRED, LIVE], present: MADE })).toEqual([
      'container env_1-web',
  ...
  it('is left alone while the environment is live, whatever it is', () => {
    expect(planned({ environments: [environment(), LIVE], present: MADE })).toEqual([])
  })

  it('is what a caller with no database takes down for one environment', () => {
    expect(ownedResources('env_live', MADE).map(leftover => `${leftover.kind} ${leftover.name}`)).toEqual([
  ...
  it('is accounted for while its row exists, and so is the port helper with its image', () => {
    expect(unattributedResources({ environments: [RETIRED, LIVE], present: MADE })).toEqual([])
  })

  it('says nothing about another install\'s live environment, whose own container names its prefix', () => {
    // A second Domo on this daemon with its own NUXT_DEV_ENV_RESOURCE_PREFIX:
    // its environment container carries `domo.envId` under a name that is not
    // ours, so neither it nor the stack it runs is anything of this one's.
    expect(unattributedResources({
      environments: [RETIRED],
      present: {
        containers: [
          { name: 'other-dev-env_theirs', owner: 'env_theirs', environment: true },
          { name: 'env_theirs-web', owner: 'env_theirs' }
        ],
        volumes: [
          { name: 'other-dev-env_theirs-workspace', owner: 'env_theirs', environment: true },
          { name: 'dind-var-lib-docker-env_theirs', owner: 'env_theirs' }
        ],
        images: [{ name: 'domo-env_theirs/docker.io/library/app:dev', owner: 'env_theirs' }]
      }
    })).toEqual([])
  })

  it('names what an environment nobody knows made, once that environment is gone too', () => {
    // A row pruned before rows were kept for good, or another install's
    // retired environment: this database cannot tell, so it says and stops.
    const present = {
      containers: [{ name: 'env_gone-web', owner: 'env_gone' }],
      volumes: [{ name: 'dind-var-lib-docker-env_gone', owner: 'env_gone' }],
      images: []
    }
    expect(unattributedResources({ environments: [RETIRED], present }))
      .toEqual(['container env_gone-web', 'volume dind-var-lib-docker-env_gone'])
    expect(planned({ environments: [RETIRED], present })).toEqual([])
  })
})
...
describe('why Docker refused', () => {
  ...
  it('asks Docker what is holding a volume, and what an image is running in', () => {
    ...
    expect(blockerArgs({ kind: 'network', name: 'env_1-default', environmentId: 'env_1' }))
      .toEqual(['ps', '--all', '--filter', 'network=env_1-default', '--format', '{{.Names}}'])
...
describe('the argv', () => {
  it('names one resource per call, and takes a container with its anonymous volumes', () => {
    ...
    expect(removeArgs({ kind: 'network', name: 'env_1-default', environmentId: 'env_1' }))
      .toEqual(['network', 'rm', 'env_1-default'])
  })
})
